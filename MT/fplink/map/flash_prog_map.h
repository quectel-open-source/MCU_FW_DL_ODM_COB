...
typedef enum {
    FPC_NONE                                    = 0x00000000,
/// Program with specified flash offset and size. Size will be truncated into 16bits.
    FPC_PROGRAM                                 = 0x00000001,
/// Erase sector with specified flash offset. The effective sector start and size
...
/// Dummy command. It can be used to determine whether FPC is alive, and the next
/// command index.
    FPC_PING                                    = 0x00000009,
/// Erase and program. It is useful for better pipeline. Also, caller specified sector/block
/// size will be used rather than configured flash layout. Only 4K and 64K are supported.
/// Caller should ensure alignment. "fcs" field in command data will be interpreted
/// as 16bits mask, to indicate whether each 16th piece should be programmed.
    FPC_ERASE_PROGRAM                           = 0x0000000A,
/// Erase with caller specified sector/block size rather than configured flash layout.
...
/// Overwrite a portion of sector(s). Ramrun should read the sector, and just replace
/// the specified data and write back. It can save time to read sector data to PC.
/// The address and size can be un-aligned. The total size (including unsignment
/// offset) must be less than (FPC_BUFFER_SIZE - 4K).
    FPC_OVERWRITE                               = 0x0000000D,
/// Read valid factory block version 1 (0xFAC000100). It will scan valid factory
/// block based on version 1 definition. When no valid factory block is found, all
/// zero block is returned. FPC will only take care of factory block management,
/// and won't look into details of factory block. Namely, only crc/version/sequence
/// are interested.
    FPC_FACTORY_READ                            = 0x0000000E,
/// Write factory block with version 1 block management. Also, block management information
/// will be built at FPC_FACTORY_READ. So, it is needed to call FPC_FACTORY_READ
/// beforehand.
    FPC_FACTORY_WRITE                           = 0x0000000F,
    FPC_SHUTDOWN                                = 0x00000010,
    FPC_IMAGE_INITIAL                           = 0x00480000,
    FPC_IMAGE_UPDATE_IMAGE                      = 0x00480001,
    FPC_IMAGE_VERIFY_IMAGE                      = 0x00480002,
    FPC_IMAGE_PROGRAM_IMAGE                     = 0x00480003,
    FPC_IMAGE_SET_UART_ID                       = 0x00480004,
    FPC_DONE                                    = 0xFFFFFF9C,
