...
int usb_get_device_property(usb_dev_handle *dev, unsigned int property,
                            void *bytes, int size)
{
    int ret;
    libusb_request request;
    ...
    memset(&request, 0, sizeof(request));
    request.timeout = LIBUSB_DEFAULT_TIMEOUT;
    request.device_property.property = property;

    if (!_usb_io_sync(dev->impl_info, LIBUSB_IOCTL_GET_DEVICE_PROPERTY,
                      &request, sizeof(request), bytes, size, &ret))
    {
        USBERR("sending get device property ioctl failed, win error: %s\n", usb_win_error_to_string());
        ret = -usb_win_error_to_errno();
    }
...
int usb_get_object_name(usb_dev_handle *dev, unsigned int objname_index,
                        void *bytes, int size)
{
    int ret;
    libusb_request request;
    ...
    memset(&request, 0, sizeof(request));
    request.timeout = LIBUSB_DEFAULT_TIMEOUT;
    request.objname.objname_index = objname_index;

    if (!_usb_io_sync(dev->impl_info, LIBUSB_IOCTL_GET_OBJECT_NAME,
                      &request, sizeof(request), bytes, size, &ret))
    {
        USBERR("sending get object name ioctl failed, win error: %s\n", usb_win_error_to_string());
        ret = -usb_win_error_to_errno();
    }
