...
public:
    ...
    int imageSendData(const QByteArray &imageData,uint32_t timeout = 30000 );
    int imageProgram(uint32_t timeout = 50000);
    int imageVerify(const QByteArray &data);
...
private:
    ...
    bool verifySram(LodData &dumpLod, uint32_t ramAddr, size_t size,uint8_t pattern,int index );
